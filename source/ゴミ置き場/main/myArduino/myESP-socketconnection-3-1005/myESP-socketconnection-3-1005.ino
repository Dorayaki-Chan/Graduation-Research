...
//#define SERIALPRINT
#define HOME//どこで使うか、wifi HOMEorLAB
...
void loop() {
    ...
    while(syomenSet==0&&digitalRead(PUSHBUTTON)==0){
    ...
    snprintf(sendDataChar,16,"0000000000");//Sで始まり,Eで終わる　エラー回避
    client.write(sendDataChar);  
        while (client.connected()) {//ボタンが1ならwifionでよし ||digitalRead(BUTTON)==0
            ...
            //送信部
            //char sendDataChar[16];
            snprintf(sendDataChar,16,"%05d%05d",xWalk,yWalk);//Sで始まり,Eで終わる　エラー回避
            client.write(sendDataChar);
            //#ifdef SERIALPRINT
