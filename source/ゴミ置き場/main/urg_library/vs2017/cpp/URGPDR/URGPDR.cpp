...
myESP-socketconnection-4-1020と対
TODO:
...
int ESPSocket(HWND hwnd) {
    ...
    */
    ...
    //MessageBox(NULL, TEXT("doにはいった"), TEXT("Test"), MB_OK);
    ...
    MessageBox(NULL, TEXT("doの前"), TEXT("Test"), MB_OK);
    ...
    do {
        ...
        if (iResult > 0) {
            if (recvCount == 0) {
                ...
                recvCount++;
                MessageBox(NULL, TEXT("受信1"), TEXT("Test"), MB_OK);
            }
            else if (recvCount == 1) {
                memcpy(iRecvDataB, recvbuf, sizeof(recvbuf));
                recvCount++;
                MessageBox(NULL, TEXT("受信2"), TEXT("Test"), MB_OK);
            }
            else if (recvCount == 2) {
                memcpy(iRecvDataC, recvbuf, sizeof(recvbuf));
                recvCount++;
                MessageBox(NULL, TEXT("受信3"), TEXT("Test"), MB_OK);
            }
            else if (recvCount == 3) {
                memcpy(iRecvDataD, recvbuf, sizeof(recvbuf));
                recvCount++;
                MessageBox(NULL, TEXT("受信4"), TEXT("Test"), MB_OK);
            }
            else {
                ...
                int xPrint, yPrint = 0;
                icount =+ 500;
                xPrint = ( walkX +icount) / 40;//TODO:実はこっちもマイナスがいるのでは？？？
                yPrint = (-1 * walkY + 10000) / 40;//y軸方向違う
               /* HDC hdc;
                PAINTSTRUCT ps;
                hdc = BeginPaint(hwnd, &ps);*/
                HPEN        hPen, hOldPen;
